...
export default function Player({
	...
	return (
        <div className=' bg-slate-50'>
			...
			<Link href={"/"} className='home-link block absolute left-4 rounded-md p-4 top-16 aspect-square w-max bg-white'>
				<svg
			...
			<div className=' flex justify-center p-4 pb-0 '>
				{file.mimeType !== 'pdf' ? (
					<div className='rounded-md overflow-hidden w-[80%] aspect-video'>
						<VideoPlayer
							video={{
								uri: file.uri,
			...
			<div className='flex justify-center'>
				<div className='flex justify-center w-[80%] px-4'>
					<h3 className=' text-xl p-5 bg-white mb-3 w-full'>{file.name}</h3>
				</div>
			</div>
			<SubTopicPanel topic={topic} subjectId={subjectId} subtopicId={subjectId} fileId={fileId} name={file.name} />
		</div>
	);
}
