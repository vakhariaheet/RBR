...
export default async function Home({children}: any) {
	...
	return (
		<main className='flex justify-center flex-col items-center p-4 gap-4'>
			...
			<div className='grid  gap-4 w-[80%]' style={{
				gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))',
			}}>
				{subjects.map((subject) => (
					<Link href={`/${subject.id}`} className='card border border-slate-100 shadow-md rounded-md overflow-hidden' key={crypto.randomUUID()} >
						<div className='image-card aspect-video bg-cover bg-center w-full' style={{
							backgroundImage: `url("/subjects/${subject.id}.jpeg")`,
						}}  key={crypto.randomUUID()}></div>
